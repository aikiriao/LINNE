...
static void LINNEPlayer_CoreAudioCallback(void *custom_data, AudioQueueRef queue, AudioQueueBufferRef buffer);

/* 初期化カウント */
static int32_t st_initialize_count = 0;
/* 初期化時のプレイヤーコンフィグ */
static struct LINNEPlayerConfig st_config = { 0, };
/* デコードしたデータのバッファ領域 */
...
void LINNEPlayer_Initialize(const struct LINNEPlayerConfig *config)
{
    ...
    AudioQueueNewOutput(&format,
            LINNEPlayer_CoreAudioCallback, NULL, CFRunLoopGetCurrent(), kCFRunLoopCommonModes, 0, &queue);
    ...
    for (i = 0; i < NUM_BUFFERS; i++) {
        ...
        /* 一発目のデータを出力 */
        LINNEPlayer_CoreAudioCallback(NULL, queue, buffers[i]);
    }
